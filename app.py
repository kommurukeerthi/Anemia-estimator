...
cur=mydb.cursor()
app=Flask(__name__)
app.secret_key="iufheuwr834689y3459379237"
...
def forgotpassword():
    if request.method=="POST":
        ...
        sql="select * from hmg where email='%s'"%(session['useremail'])
        cur.execute(sql)
        dc = cur.fetchall()
        print(dc)
        if dc !=[]:
            return render_template('forgotpassword.html',info='authorized')

    return render_template("forgotpassword.html",info='')
...
def updatepassword():
    if request.method=="POST":
        ...
        if password1 == password2:
            sql="update hmg set password='%s' where email='%s'"%(password1,session['useremail'])
            cur.execute(sql)
            mydb.commit()
...
def prediction():
    if request.method=="POST":
        val=float(request.form['d'])
        # if f1=="male":
        #     val=0
        # else:
        #     val=1
        print(val)
        f2=float(request.form['age'])
        f3=float(request.form['rbc'])
        f4=float(request.form['pcv'])
        f5=float(request.form['mcv'])
        f6=float(request.form['mch'])
        f7=float(request.form['mchc'])
        f8=float(request.form['rdw'])
        f9=float(request.form['tlc'])
        f10=float(request.form['plt'])
        print(f10)
        print(type(f10))
